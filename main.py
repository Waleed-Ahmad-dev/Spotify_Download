...
def main() -> None:
    ...
    parser.add_argument("--playlist",  type=str, metavar="NAME", help="Generate an .m3u playlist with this name")
    parser.add_argument("--dedupe",    type=str, metavar="DIR",  help="Remove duplicates in a specified directory")
    ...
    if (args.download or args.all) and not args.format:
        if questionary:
            ...
            console.print(
                "[yellow]⚠️  'questionary' not found – defaulting to MP3. "
    ...
    if args.download or args.all:
        ...
        if downloaded:
            ...
            # ── Summary table ────────────────────────────────────────────
            console.print()
            summary = Table(title="🎉 Run Summary", show_header=True, header_style="bold magenta")
            ...
            if args.organize:
                summary.add_row("Folder Organization", "Enabled (By Artist)")
            if args.playlist:
